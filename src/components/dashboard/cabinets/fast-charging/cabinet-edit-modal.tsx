...
import { X, Zap, Map, Building2, MapPin } from "lucide-react";
import { Cabinet, EditCabinetForm, CabinetStatus } from "../types";
import { STATUS_CFG } from "./cabinet-card";

const ACCENT = "#F59E0B";

interface Props { cabinet: Cabinet; onClose: () => void; onSave: (id: string, form: EditCabinetForm) => void; }

export default function CabinetEditModal({ cabinet, onClose, onSave }: Props) {
  const [form, setForm] = useState<EditCabinetForm>({
    lat: String(cabinet.lat), lng: String(cabinet.lng),
    address: cabinet.address, city: cabinet.city,
    province: cabinet.province, status: cabinet.status,
  });

  const fields: { key: keyof Omit<EditCabinetForm, "status">; label: string; placeholder: string; icon: React.ReactNode; half?: boolean }[] = [
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-md mx-4 shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2">
              {(["active", "offline", "faulty"] as CabinetStatus[]).map(s => (
                <button key={s} onClick={() => setForm({ ...form, status: s })}
                  className={`flex-1 py-2 rounded-xl text-xs font-semibold border transition-all ${form.status === s ? STATUS_CFG[s].badge : "bg-gray-50 text-gray-400 border-gray-200 hover:bg-gray-100"}`}>
                  {STATUS_CFG[s].label}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {fields.map(f => (
              <div key={f.key} className={f.half ? "" : "col-span-2"}>
                ...
                <input value={form[f.key] as string} onChange={e => setForm({ ...form, [f.key]: e.target.value })} placeholder={f.placeholder}
        ...
        <div className="flex gap-3 px-6 pb-6">
          ...
          <button onClick={() => { onSave(cabinet.id, form); onClose(); }} className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white hover:opacity-90 transition-all" style={{ backgroundColor: "#1C1FC1" }}>Save Changes</button>
        </div>
      </div>
    </div>
  );
