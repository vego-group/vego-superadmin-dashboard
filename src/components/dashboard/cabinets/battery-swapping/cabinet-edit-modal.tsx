...
import { X, Battery, Map, Building2, MapPin, Loader2, AlertCircle } from "lucide-react";
      ...
      <div className="bg-white rounded-2xl w-full max-w-md mx-4 shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${ACCENT}18`, color: ACCENT }}>
          ...
          <button onClick={handleClose} disabled={isLoading} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 transition-colors disabled:opacity-50">
            <X className="h-4 w-4" />
          </button>
        </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2">
              {(["active", "offline", "faulty"] as CabinetStatus[]).map((s) => (
                <button key={s} onClick={() => setForm({ ...form, status: s })} disabled={isLoading}
                  className={`flex-1 py-2 rounded-xl text-xs font-semibold border transition-all disabled:opacity-50 ${
                    form.status === s ? STATUS_CFG[s].badge : "bg-gray-50 text-gray-400 border-gray-200 hover:bg-gray-100"
                  }`}>
                  {STATUS_CFG[s].label}
                </button>
              ))}
        ...
        <div className="flex gap-3 px-6 pb-6">
          <button onClick={handleClose} disabled={isLoading} className="flex-1 py-2.5 rounded-xl text-sm font-medium text-gray-500 bg-gray-100 hover:bg-gray-200 transition-colors disabled:opacity-50">
            Cancel
          </button>
