  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="px-3 sm:px-4 py-2 sm:py-3 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div>
          <h3 className="text-sm sm:text-base font-semibold text-gray-900">Cabinets Map</h3>
          <p className="text-[10px] sm:text-xs text-gray-500 mt-0.5">
            {validCabinets.length} cabinets displayed on map
          </p>
        </div>
...
<MapContainer
  center={mapCenter}
  zoom={mapZoom}
  style={{ height: "100%", width: "100%", zIndex: 1 }}  // ← أضف zIndex: 1
...
>
      ...
      {validCabinets.length === 0 && cabinets.length > 0 && (
        <div className="p-4 sm:p-6 text-center">
          <p className="text-xs sm:text-sm text-gray-500">
            ⚠️ No cabinets with valid coordinates found. Please add coordinates to display them on map.
          </p>
        </div>
      )}
      ...
      {cabinets.length === 0 && (
        <div className="p-4 sm:p-6 text-center">
          <p className="text-xs sm:text-sm text-gray-500">No cabinets to display on map.</p>
        </div>
      )}
    </div>
  );
