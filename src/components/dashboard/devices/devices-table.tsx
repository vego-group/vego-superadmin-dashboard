        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50/50">
              <th className="px-6 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-widest">{t("Device Details", "تفاصيل الجهاز")}</th>
              <th className="px-4 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-widest">{t("Type", "النوع")}</th>
              <th className="px-4 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-widest">{t("Location", "الموقع")}</th>
              <th className="px-4 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-widest">{t("Status", "الحالة")}</th>
              <th className="px-4 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-widest">{t("Capacity", "السعة")}</th>
              <th className="px-4 py-4 text-[11px] font-bold text-gray-500 uppercase tracking-widest">{t("Registered", "تاريخ التسجيل")}</th>
              <th className="px-6 py-4 text-right text-[11px] font-bold text-gray-500 uppercase tracking-widest">{t("Action", "إجراء")}</th>
            </tr>
          </thead>
              ...
              return (
                <tr key={device.id} className="hover:bg-gray-50/80 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-gray-400 font-medium truncate max-w-[150px]">
                        {device.name || t("Unnamed Device", "جهاز بدون اسم")}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-baseline gap-1">
                      ...
                      <span className="text-xs text-gray-400 font-medium">
                        / {device.slots} {device.type === "cabinet" ? t("Bat.", "بطارية") : t("Ports", "منفذ")}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button 
                      className="inline-flex items-center justify-center h-8 w-8 rounded-lg border border-gray-100 bg-white text-gray-400 hover:text-indigo-600 hover:border-indigo-100 hover:shadow-sm transition-all"
          ...
          return (
            <div key={device.id} className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 bg-gray-50 rounded-xl p-3">
                <div>
                  <p className="text-[10px] text-gray-400 uppercase font-bold">{t("City", "المدينة")}</p>
                  <p className="text-xs font-semibold text-gray-700">{device.city}</p>
                </div>
                <div>
                  <p className="text-[10px] text-gray-400 uppercase font-bold">{t("Capacity", "السعة")}</p>
                  <p className="text-xs font-semibold text-gray-700">
                    {device.type === "cabinet" ? (device.availableSlots ?? 0) : device.slots}/{device.slots}
                  </p>
                </div>
                <div className="col-span-2">
                  <p className="text-[10px] text-gray-400 uppercase font-bold">{t("Location", "الموقع")}</p>
                  <p className="text-xs font-semibold text-gray-700 truncate">{device.location}</p>
                </div>
              </div>
            </div>
          );
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                ...
                >
                  {p.toLocaleString(lang === "ar" ? "ar-SA" : "en-US")}
                </button>
              ))}
            </div>
            <button 
