...
export default function SalesStaffIndex() {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    setError(null);
    try {
      const res  = await fetch("/api/proxy/staff?role=sales", {
        headers: { Accept: "application/json" },
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.message || `Error ${res.status}`);
      const list = json.data?.data ?? json.data ?? [];
      setMembers(list.map(normaliseMember));
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to fetch sales staff");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/proxy/staff/${deleting.id}`, {
        method: "DELETE",
        headers: { Accept: "application/json" },
      });
      if (!res.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-semibold text-gray-800">Sales Management</h1>
          <p className="text-sm text-gray-500 mt-1">Manage sales team members</p>
        </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-20 text-gray-400 gap-2 text-sm">
          <RefreshCw className="h-4 w-4 animate-spin" /> Loading sales staff…
        </div>
      ) : (
        <SalesTable
          members={filtered}
      ...
      <Dialog open={!!deleting} onOpenChange={() => setDeleting(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete Sales Member</DialogTitle>
            <DialogDescription>
