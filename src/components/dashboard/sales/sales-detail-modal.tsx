...
export default function SalesDetailModal({ member, onClose }: Props) {
  ...
  const rows = [
    { label: "Phone",          value: member.phone ?? "—",   icon: <Phone className="h-3.5 w-3.5" />         },
    { label: "Email",          value: member.email ?? "—",   icon: <Mail className="h-3.5 w-3.5" />          },
    { label: "Phone Verified", value: member.phone_verified ? "Yes" : "No",
      icon: member.phone_verified ? <CheckCircle2 className="h-3.5 w-3.5 text-green-500" /> : <XCircle className="h-3.5 w-3.5 text-gray-400" /> },
    { label: "Language",       value: member.language.toUpperCase(), icon: null },
    { label: "Joined",         value: new Date(member.created_at).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }), icon: null },
  ];
      ...
      <div className="bg-white rounded-2xl w-full max-w-md mx-4 shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Sales Team · #{member.id}</p>
            </div>
          </div>
        ...
        <div className="px-6 pb-6">
          <button onClick={onClose}
            className="w-full py-2.5 rounded-xl text-sm font-medium text-gray-500 bg-gray-100 hover:bg-gray-200 transition">
            Close
          </button>
        </div>
      </div>
