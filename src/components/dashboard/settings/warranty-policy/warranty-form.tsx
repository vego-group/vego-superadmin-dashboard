  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="px-6 py-5 border-b border-gray-100">
        <h2 className="text-base font-semibold text-gray-900">Warranty Policy Configuration</h2>
        <p className="text-xs text-gray-400 mt-1">Set up and manage your warranty policy information</p>
      </div>
      ...
      <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50/50">
        ...
        <button onClick={handleSave} disabled={isSaving}
          ...
          {isSaving ? <><Loader2 className="h-4 w-4 animate-spin" />Saving...</>
                    : <><Save className="h-4 w-4" />{saved ? "Saved ✓" : "Save Policy"}</>}
        </button>
      </div>
    </div>
  );
