...
import {
  ...
  ImageIcon, Facebook, Instagram, Linkedin, Twitter, Loader2,
} from "lucide-react";
...
const SocialField = ({ icon, label, value, onChange, placeholder }: {
  icon: React.ReactNode; label: string; value: string;
  onChange: (v: string) => void; placeholder: string;
}) => (
  <div>
    <label className="flex items-center gap-1.5 text-xs font-medium text-gray-600 mb-1.5">{icon}{label}</label>
...
export default function AboutForm() {
  ...
  const [social, setSocial] = useState({ facebook: "", instagram: "", x: "", linkedin: "" });

  // ── Fetch ──────────────────────────────────────────────────────────────────
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res  = await fetch("/api/proxy/about-us", {
          headers: { Accept: "application/json" },
        });
        const json = await res.json();
        const d    = json?.data;
        if (!d) return;
        setCompanyTitle(d.company_title ?? "");
        setSocial({
        ...
        if (editorRef.current && d.about_us_description) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // multipart/form-data عشان فيه صورة
      const formData = new FormData();
      formData.append("company_title",        companyTitle);
      formData.append("about_us_description", description);
      formData.append("facebook_url",         social.facebook);
      formData.append("instagram_url",        social.instagram);
      formData.append("x_url",                social.x);
      formData.append("linkedin_url",         social.linkedin);
      if (logoFile) formData.append("company_logo", logoFile);

      const res = await fetch("/api/proxy/about-us", {
        method: "POST",
        body: formData,
        // لا تحط Content-Type عشان الـ browser يحطه تلقائياً مع الـ boundary
      });
  ...
  const handleCancel = async () => {
    try {
      const res  = await fetch("/api/proxy/about-us", { headers: { Accept: "application/json" } });
      const json = await res.json();
      const d    = json?.data;
      if (!d) return;
      setCompanyTitle(d.company_title ?? "");
      setSocial({ facebook: d.facebook_url ?? "", instagram: d.instagram_url ?? "", x: d.x_url ?? "", linkedin: d.linkedin_url ?? "" });
      if (d.company_logo_url) setLogoPreview(d.company_logo_url);
      setLogoFile(null);
      if (editorRef.current) editorRef.current.innerHTML = d.about_us_description ?? "";
    } catch {}
  };
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="px-6 py-5 border-b border-gray-100">
        <h2 className="text-base font-semibold text-gray-900">{t("Company Information", "معلومات الشركة")}</h2>
        <p className="text-xs text-gray-400 mt-1">{t("Update your company's about us information and social media presence.", "قم بتحديث معلومات عن الشركة وحسابات التواصل الاجتماعي.")}</p>
      </div>
      ...
      <div className="p-6 flex flex-col gap-6">
        ...
        {/* Company Title */}
        <div>
          <label className="block text-sm text-gray-700 mb-1.5">{t("Company Title", "اسم الشركة")}</label>
          <input type="text" value={companyTitle} onChange={(e) => setCompanyTitle(e.target.value)}
            placeholder={t("Enter company title", "أدخل اسم الشركة")}
        ...
        {/* Description */}
        <div>
          <label className="block text-sm text-gray-700 mb-1.5">{t("About Us Description", "وصف عن الشركة")}</label>
          <div className="border border-gray-200 rounded-lg overflow-hidden focus-within:border-indigo-400 focus-within:ring-1 focus-within:ring-indigo-100 transition-colors">
            <div className="flex items-center flex-wrap gap-0.5 px-3 py-2 border-b border-gray-200 bg-gray-50">
              {toolbarGroups.map((group, gi) => (
                <div key={gi} className="flex items-center gap-0.5">
                  {gi > 0 && <Divider />}
                  {group.map((btn) => (
                    <ToolbarBtn key={btn.title} title={btn.title} active={isActive(btn.cmd)} onClick={() => exec(btn.cmd, btn.value)}>
                      {btn.icon}
                    </ToolbarBtn>
                  ))}
                </div>
            ...
            <div ref={editorRef} contentEditable suppressContentEditableWarning
              className="min-h-[160px] max-h-[300px] overflow-y-auto px-4 py-3 text-sm text-gray-600 leading-relaxed focus:outline-none whitespace-pre-wrap"
        ...
        {/* Logo */}
        <div>
          <label className="block text-sm text-gray-700 mb-1.5">{t("Company Logo", "شعار الشركة")}</label>
          <div onDragOver={(e) => e.preventDefault()} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()}
            ...
            <p className="text-xs text-indigo-500">
              {logoPreview ? t("Click to change logo", "انقر لتغيير الشعار") : t("Drop your logo here or click to browse", "أسقط الشعار هنا أو انقر للتصفح")}
            </p>
        ...
        {/* Social Media */}
        <div>
          <h3 className="text-sm font-semibold text-gray-800 mb-4">{t("Social Media Links", "روابط التواصل الاجتماعي")}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <SocialField icon={<Facebook className="h-3.5 w-3.5 text-blue-600" />} label="Facebook"
              value={social.facebook} onChange={(v) => setSocial({ ...social, facebook: v })} placeholder="https://facebook.com/yourpage" />
            <SocialField icon={<Instagram className="h-3.5 w-3.5 text-pink-500" />} label="Instagram"
              value={social.instagram} onChange={(v) => setSocial({ ...social, instagram: v })} placeholder="https://instagram.com/yourprofile" />
            <SocialField icon={<Twitter className="h-3.5 w-3.5 text-gray-800" />} label="X"
              value={social.x} onChange={(v) => setSocial({ ...social, x: v })} placeholder="https://x.com/yourhandle" />
            <SocialField icon={<Linkedin className="h-3.5 w-3.5 text-blue-700" />} label="LinkedIn"
              value={social.linkedin} onChange={(v) => setSocial({ ...social, linkedin: v })} placeholder="https://linkedin.com/company/yourcompany" />
          </div>
        </div>
      </div>
