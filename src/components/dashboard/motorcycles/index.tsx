...
export default function MotorcyclesIndex() {
  ...
  const fetchMotorcycles = useCallback(async () => {
    setIsLoading(true);
    try {
      const res  = await fetch("/api/proxy/motorcycles", {
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1">Manage fleet motorcycles and battery assignments</p>
        </div>
        <button
      ...
      <MotorcyclesTable
        ...
        isLoading={isLoading}
        onAssignBattery={setAssignTarget}
      />

      {assignTarget && (
        <AssignBatteryModal
          motorcycle={assignTarget}
