...
interface Props {
  ...
  onUnassign: (m: Motorcycle) => void;
  selectedIds: Set<number>;
  onToggle: (id: number) => void;
  onToggleAll: () => void;
}

export default function MotorcyclesTable({ motorcycles, isLoading, onAssignBattery, onUnassign, selectedIds, onToggle, onToggleAll }: Props) {
  ...
  const pageIds = motorcycles.map((m) => m.id);
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <table className="w-full" dir={isRtl ? "rtl" : "ltr"}>
          ...
          <tbody className="divide-y divide-gray-50">
            {motorcycles.map((m) => {
              ...
              return (
                <tr key={m.id} className={`transition-colors ${isSelected ? "bg-indigo-50/40" : "hover:bg-gray-50/50"}`}>
                  <td className="px-4 py-3">
                    <input type="checkbox" checked={isSelected} onChange={() => onToggle(m.id)}
                      className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer accent-indigo-600" />
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      {getAssignment(m) !== "unassigned" && (
                        <button onClick={() => onUnassign(m)} title={t("Unassign","إلغاء التعيين")}
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-orange-600 border border-orange-200 hover:bg-orange-50 transition whitespace-nowrap">
      ...
      <div className="md:hidden divide-y divide-gray-100">
        {motorcycles.map((m) => {
          const cfg = statusCfg[m.status];
          return (
            <div key={m.id} className={`p-4 space-y-3 ${selectedIds.has(m.id) ? "bg-indigo-50/40" : ""}`}>
              ...
              <div className="flex gap-2">
                <button onClick={() => onAssignBattery(m)}
                  className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-indigo-200 text-indigo-600 text-xs font-medium hover:bg-indigo-50 transition">
                  <Zap className="h-3.5 w-3.5" />
                  {m.battery ? t("Reassign Battery","إعادة تعيين البطارية") : t("Assign Battery","تعيين البطارية")}
                </button>
                {getAssignment(m) !== "unassigned" && (
                  <button onClick={() => onUnassign(m)}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-orange-200 text-orange-600 text-xs font-medium hover:bg-orange-50 transition">
                    <Unlink className="h-3.5 w-3.5" />
                    {t("Unassign","إلغاء التعيين")}
                  </button>
                )}
